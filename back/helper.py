...
from models import Country, TimeZone, Language, Currency, City
...
def filter_by_name(model, query, field, names):
    ...
    if hasattr(
        model, field
    ):  # This means that the attribute is part of an object inside the model (i.e. continent for filtering cities)
        ...
        return query.join(model.timezone, aliased=True).filter(
            sqlalchemy.or_(*condition)
        )
    elif field == "lang":
        condition = [getattr(Language, "name") == name for name in names]
        return query.join(model.language, aliased=True).filter(
            sqlalchemy.or_(*condition)
        )
    elif field == "country":
        condition = [getattr(Country, "officialName") == name for name in names]
        return query.join(model.country, aliased=True).filter(
            sqlalchemy.or_(*condition)
        )
    else:
...
def sort(model, query, _, attr_list):
    ...
    if model is City and (
        attr_list[0] == "population" or attr_list[0] == "-population"
    ):
            ...
            if type(searchable[model.__name__][att]) == list:
                ...
                try:
                    condition = sqlalchemy.or_(
                        condition,
                        getattr(model, att).any(
                            getattr(*relation).ilike("%{}%".format(word))
                        ),
                    )
                except InvalidRequestError:
                    condition = sqlalchemy.or_(
                        condition,
                        getattr(model, att).has(
                            getattr(*relation).ilike("%{}%".format(word))
                        ),
                    )
                except:
                ...
                try:
                    ...
                    if relation == "string":
                        condition = sqlalchemy.or_(
                            condition, getattr(model, att).ilike("%{}%".format(word))
                        )
                    else:
                        condition = sqlalchemy.or_(
                            condition,
                            cast(getattr(model, att), sqlalchemy.String).ilike(
                                "%{}%".format(word)
                            ),
                        )
                except:
